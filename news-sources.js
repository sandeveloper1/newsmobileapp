...
// Modify existing loadNewsSources to support category tabs
async function loadNewsSources(isFavoritesPage = false) {
    try {
        ...
        // Get favorites and custom newspapers
        const favorites = FavoriteManager.getFavorites();
        const customNewspapers = CustomNewspaperManager.getCustomNewspapers();
        ...
        if (isFavoritesPage) {
            // Only show sources that are in the favorites list
            sourcesToDisplay = sourcesToDisplay.filter(source => 
                favorites.some(fav => fav === source.id)
            );
        }

        // Get news sources container
        const newsSourcesContainer = document.getElementById('newsSources');
        if (!newsSourcesContainer) {
            console.error('News sources container not found');
            return;
        ...
        sourcesToDisplay.forEach(source => {
            // Always mark as favorite on favorites page
            const sourceCard = createSourceCard(source, isFavoritesPage);
            newsSourcesContainer.appendChild(sourceCard);
        });

        // Handle empty state for favorites page
        if (isFavoritesPage && sourcesToDisplay.length === 0) {
            newsSourcesContainer.innerHTML = `
        ...
        // Populate category tabs if not on favorites page
        if (!isFavoritesPage) {
            const categoryTabs = document.getElementById('categoryTabs');
            if (categoryTabs && categoryTabs.children.length <= 1) {
                // Combine categories from data and custom sources
                const allCategories = new Set([
                    ...data.categories,
                    ...new Set(sourcesToDisplay.map(source => source.category))
                ]);
...
// Call data loading on script load
